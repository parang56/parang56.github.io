...
int main(void)
{
	...
	// getch() : 키 입력 시 바로 동작을 합니다.
	
	int i=0;
	while(1)
	{
		c = getch();
		if (c==10) // Enter -> 비밀번호 입력 종료
		{
			password[i] = '\0';
			break;
		}
	...
	if (strcmp(password, "skehzheld") == 0) //비밀번호 일치
	{
		...
		// a+b -> 파일이 없으면 생성, 파일이 있으면 append 를 한다 (뒤에서부터 내용을 추가)
		if(file == NULL)
		{
			printf("파일 열기 실패\n");
			return 1;
		...
		printf("\n\n 내용을 계속 작성하세요! 종료하시려면 EXIT 를 입력해주세요!\n\n");
		while(1)
		{
